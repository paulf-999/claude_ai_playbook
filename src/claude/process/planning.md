...
- **`"Implement the following plan:"` is auto-generated by Claude Code when exiting plan mode.** It is not a confirmed go-ahead to begin making file changes. Treat it the same as any non-trivial task: enter plan mode, present the approach, and wait for explicit confirmation before touching any file.
