...
- **Generic patterns** — `~/.claude/skills/_atlassian_skills/create_confluence_page/patterns/<pattern_name>.md`
- **Data platform patterns** — `~/.claude/skills/_atlassian_skills/create_confluence_page/patterns/data_platform/<pattern_name>.md`
...
1. **Write the draft** — Save the page content as a markdown file:
   - Directory: `~/confluence_drafts/`
   - Filename: `<page-title-slug>_draft.md` (lowercase, words separated by underscores, no special characters)
   - Create the directory if it does not exist (`mkdir -p ~/confluence_drafts/`)
...
2. **Ask for feedback** — Inform the user of the file path and request review:
   > "Draft written to `~/confluence_drafts/<filename>`. Please review and let me know any changes before I publish to Confluence."
...
5. **Optional review** — After publishing, ask:
   ...
   If yes, invoke `/review_confluence_page` with the newly published page ID.
