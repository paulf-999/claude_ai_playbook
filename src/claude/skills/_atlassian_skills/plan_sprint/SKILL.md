---
name: plan_sprint
description: Plan a sprint — review Jira capacity, identify committed and missing tickets, confirm scope, and optionally update or create the sprint goals Confluence page. Requires the Atlassian MCP server to be enabled (`make enable_mcp server=Atlassian`, then restart Claude Code).
version: 0.1.0
maturity: draft
tags:
  criticality: should
...
Plan a sprint. Work through the steps below in order, waiting for user input at each phase before proceeding.
...
```
project = <PROJECT_KEY> AND sprint = "<TEAM> Sprint <number>" ORDER BY assignee ASC
```

For each issue, capture: key, summary, status, story points (if set), assignee.

If no sprint name is known, try `sprint in openSprints() AND project = <PROJECT_KEY>`.
...
Create each issue using `createJiraIssue` in project `<PROJECT_KEY>` with type `Story`. Confirm each ticket key back to the user.

---

## Step 6 — Update Confluence page (optional)

> **Sub-agent:** All Confluence page work in this step must be performed as the **technical_writer** sub-agent. Switch to that persona before proceeding with any page creation or update.

If the user wants to update the existing sprint goals page:

Read the current page in ADF format. Add or update rows in the Must, Should, Stretch Goals, Blocked, and Not Picked Up This Sprint sections to reflect the confirmed sprint scope. Use `updateConfluencePage` with `contentFormat: adf`.

When updating an existing page, apply the formatting conventions from the `data_platform_sprint_goals` pattern: status lozenges in the Status column (In Progress → yellow, Planned → neutral, Blocked → red), `expand` components for capacity breakdowns, `tableHeader` cells for the Theme column, and one linked bullet per Jira ticket in the Jira column.

If no page exists yet, offer to create one using the `data_platform_sprint_goals` pattern from `/create_confluence_page`.

**Publishing location for new pages:**
- Space: `<CONFLUENCE_SPACE_KEY>`
- Parent page: `<SPRINT_GOALS_PARENT_PAGE_ID>`

Publish the page as a child of the sprint goals parent page — do not publish at the space root or under any other parent.
