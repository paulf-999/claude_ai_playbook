...
Check `~/.claude/notes/` for a previous catchup file (e.g. `catchup_<date>.md`). If one exists, surface the most recent one — it provides continuity context (open items, blockers carried forward, tone that landed well).
...
Use the template at `~/.claude/notes/catchup_template.md` as the output format.
...
1. Save it to `~/.claude/notes/catchup_<YYYY-MM-DD>.md`
2. Add a row to the past outputs table in `~/.claude/notes/catchup_template.md`:

| Date | File |
|---|---|
| `YYYY-MM-DD` | `catchup_<YYYY-MM-DD>.md` |
