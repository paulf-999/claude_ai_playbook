...
not_for:
  - reviewing a PR — use /git_review_pr instead
  - posting a Teams notification — use /git_notify_pr instead
output:
  type: mixed
...
> - **(r)** Post a Claude code review comment (`/git_review_pr`)
> - **(n)** Send a Teams notification (`/git_notify_pr`)
>
...
- **`git_review_pr`**: read `~/.claude/skills/_git_skills/git_review_pr/SKILL.md` and execute all phases for the newly-created PR number
- **`git_notify_pr`**: read `~/.claude/skills/_git_skills/git_notify_pr/SKILL.md` and execute it for the newly-created PR number
