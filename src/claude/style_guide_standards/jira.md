# 🎫 Jira Style Guide & Standards

Defines standards for the team Jira project — field requirements, ticket structure, component and sprint assignment, and hygiene expectations.

---

## 📋 Child pages

| File | Purpose |
|------|---------|
| [`jira/ticket_conventions.md`](jira/ticket_conventions.md) | Summary naming, description structure, acceptance criteria, and issue type usage |
| [`jira/field_standards.md`](jira/field_standards.md) | Required fields, default values, custom field IDs, component IDs, and label conventions |
| [`jira/definition_of_ready.md`](jira/definition_of_ready.md) | Definition of Ready checklist — blocking and recommended checks before sprint entry |
| [`jira/sprint_planning.md`](jira/sprint_planning.md) | Sprint ID mapping, quarter/component mapping, parent epics, and capacity conventions |

---

## 🏗️ Core principles

- **Completeness** — all required fields set before a ticket enters the sprint: priority, story points, assignee, sprint, description
- **Scannability** — bullet points throughout; no prose walls in description or acceptance criteria
- **Consistency** — standard components, labels, and description structure on every ticket
- **Traceability** — a team-specific label on all Claude-authored tickets; enables hygiene checks and filtering

---

## 📥 Imports

@./jira/ticket_conventions.md
@./jira/field_standards.md
@./jira/definition_of_ready.md
@./jira/sprint_planning.md
