...
def test_find_all_components_excludes_patches(tmp_path: Path):
    """Files under patches/ are excluded from both lists."""
    patches_dir = tmp_path / "patches"
    patches_dir.mkdir()
    patches_file = patches_dir / "draft-skill.md"
    post = frontmatter.Post("wip body", **VALID_METADATA)
    patches_file.write_text(frontmatter.dumps(post))

    tagged, untagged = find_all_components(tmp_path)
    assert patches_file not in tagged
    assert patches_file not in untagged
